...
3. **One inline `<script>` block** (from line ~849) containing all app logic, in roughly this order:
   ...
   - Format parsers: KML (`parseKml`, `buildStyleMap`, `parseStyleEl`), KMZ (via JSZip), GeoJSON (`addGeoJsonFeatures`), Shapefile (via shpjs, `groupShapefileParts`). `parseKml`'s per-placemark color follows a priority chain: SEA status color (if `Estado` is present — the intentional exception, so "Coloreado inteligente" keeps working even on a Placemark that also has its own KML `Style`) → the Placemark's own native KML color, from `getStyleForPlacemark`/`parseStyleEl` (stroke via `LineStyle`, fill via `PolyStyle`, independently — `getStyleForPlacemark` leaves `style.color`/`style.fillColor` `undefined`, not defaulted, specifically so this layer can tell "no Style defined" apart from "explicitly styled") → the `líneas/ductos` folder-name heuristic → the file's auto-assigned palette color, only as the final fallback for placemarks with no styling info at all.
